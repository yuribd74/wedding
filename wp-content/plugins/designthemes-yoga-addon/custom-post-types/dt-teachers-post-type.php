...
if( !class_exists('DTYogaTeachersPostType') ) {
	...
	class DTYogaTeachersPostType {
		...
		function dt_admin_init() {
			...
			add_action ( 'add_meta_boxes', array ( $this, 'dt_add_yoga_teacher_meta_box' ) );
		}		

		function createPostType() {

			$postslug 		= 'dt_teachers';
			$singular_name  = __('Teacher', 'veda-yoga');
			$plural_name    = __('Teachers', 'veda-yoga');

			if( function_exists( 'veda_opts_get' ) ) :
				$postslug 		=	veda_opts_get( 'single-teacher-slug', 'dt_teachers' );
				$singular_name  =	veda_opts_get( 'singular-teacher-name', __('Teacher', 'veda-yoga') );
				$plural_name	=	veda_opts_get( 'plural-teacher-name', __('Teachers', 'veda-yoga') );
			endif;
			...
			register_post_type ( 'dt_yoga_teachers', $args );
		}

		function dt_add_yoga_teacher_meta_box() {

			add_meta_box ( 'dt-yoga-teacher-metabox', __ ( 'Default Options', 'veda-yoga' ), array (
				$this,
				'dt_default_metabox' 
			), 'dt_yoga_teachers', 'normal', 'default' );			
		}

		function dt_default_metabox() {
			include_once plugin_dir_path ( __FILE__ ) . 'metaboxes/dt_yoga_teacher_metabox.php';
		}		
		...
		function save_post_meta( $post_id ) {
			...
			if( key_exists( 'dt_theme_yoga_teacher_meta_nonce',$_POST ) ) :
				if ( ! wp_verify_nonce( $_POST['dt_theme_yoga_teacher_meta_nonce'], 'dt_theme_yoga_teacher_nonce') ) return;
			endif;

			if ( (key_exists('post_type', $_POST)) && ('dt_yoga_teachers' == $_POST['post_type']) ) :
		...
		function dt_template_include($template) {
			...
			if (is_singular( 'dt_yoga_teachers' )) {
				if (! file_exists ( get_stylesheet_directory () . '/single-dt_yoga_teachers.php' )) {
					$template = plugin_dir_path ( __FILE__ ) . 'templates/single-dt_yoga_teachers.php';
				}
			}
