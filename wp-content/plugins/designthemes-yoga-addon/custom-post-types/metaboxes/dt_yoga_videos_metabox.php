<?php global $post;
	...
	echo '<input type="hidden" name="dt_theme_yoga_video_meta_nonce" value="'.wp_create_nonce('dt_theme_yoga_video_nonce').'" />';?>
...
<div class="custom-box">
    <div class="column one-sixth">
        <label><?php esc_html_e('Video URL','veda-yoga');?> </label>
    </div>
    <div class="column five-sixth last">
        <?php $v = array_key_exists("video-url",$post_settings) ?  $post_settings['video-url'] : '';?>
        <input id="video-url" name="video-url" class="large" type="text" value="<?php echo $v;?>" style="width:100%;" />
        <p class="note"> <?php esc_html_e("You can given video url.",'veda-yoga');?> </p>
    </div>
</div>

<div class="custom-box">
    <div class="column one-sixth">
        <label><?php esc_html_e('Sub Title','veda-yoga');?> </label>
    </div>
    <div class="column five-sixth last">
        <?php $v = array_key_exists("sub-title",$post_settings) ?  $post_settings['sub-title'] : '';?>
        <input id="sub-title" name="sub-title" class="large" type="text" value="<?php echo $v;?>" style="width:100%;" />
        <p class="note"> <?php esc_html_e("You can give a sub title for a video.",'veda-yoga');?> </p>
    </div>
</div>
...
<div class="custom-box">
    ...
    <div class="column five-sixth last">
        <select name="styles[]" class="dt-chosen-select"  multiple="multiple">
            <option value=""></option><?php
                $ystyles_query = new WP_Query();
                $ystyles_query->query( array(
                ...
                if( $ystyles_query->have_posts() ) {
                    while( $ystyles_query->have_posts() ) {
                        $ystyles_query->the_post();
                        $the_id = get_the_ID();
                        $title = get_the_title();
        ...
        <p class="note"> <?php esc_html_e("Choose Style",'veda-yoga');?> </p>
    </div>
</div>
<!-- Style -->
...
<div class="custom-box">
    <div class="column one-sixth">
        <label><?php esc_html_e('Premium Videos','veda-yoga');?> </label>
    </div>
    <div class="column five-sixth last"><?php
        $switchclass = array_key_exists("is-premium-video",$post_settings) ? 'checkbox-switch-on' :'checkbox-switch-off';
        $checked = array_key_exists("is-premium-video",$post_settings) ? ' checked="checked"' : '';?>
        <div data-for="dttheme-is-premium-video" class="checkbox-switch <?php echo esc_attr($switchclass);?>"></div>
        <input id="dttheme-is-premium-video" class="hidden" type="checkbox" name="is-premium-video" value="true"
            <?php echo $checked;?>/>
        <p class="note"> <?php esc_html_e('Yes! to set Premium Video','veda-yoga');?> </p>
    </div>
</div><!-- Premium Videos -->