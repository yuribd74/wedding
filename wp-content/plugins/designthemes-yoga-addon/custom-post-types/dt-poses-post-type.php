...
if( !class_exists('DTYogaPosesPostType') ) {
	...
	class DTYogaPosesPostType {
		...
		function dt_admin_init() {
			...
			add_action ( 'add_meta_boxes', array ( $this, 'dt_add_yoga_pose_meta_box' ) );

		}

		function dt_add_yoga_pose_meta_box() {

			add_meta_box ( 'dt-yoga-pose-metabox', __ ( 'Default Options', 'veda-yoga' ), array (
				$this,
				'dt_default_metabox' 
			), 'dt_yoga_poses', 'normal', 'default' );			
		}

		function dt_default_metabox() {
			include_once plugin_dir_path ( __FILE__ ) . 'metaboxes/dt_yoga_poses_metabox.php';
		}

		function createPostType() {

			$postslug 		= 'dt_poses';
			$singular_name  = __('Pose', 'veda-yoga');
			$plural_name    = __('Poses', 'veda-yoga');

			if( function_exists( 'veda_opts_get' ) ) :
				$postslug 		=	veda_opts_get( 'single-pose-slug', 'dt_poses' );
				$singular_name  =	veda_opts_get( 'singular-pose-name', __('Pose', 'veda-yoga') );
				$plural_name	=	veda_opts_get( 'plural-pose-name', __('Poses', 'veda-yoga') );
			endif;
			...
			register_post_type ( 'dt_yoga_poses', $args );
		}
		...
		function save_post_meta( $post_id ) {
			...
			if( key_exists( 'dt_theme_yoga_pose_meta_nonce',$_POST ) ) :
				if ( ! wp_verify_nonce( $_POST['dt_theme_yoga_pose_meta_nonce'], 'dt_theme_yoga_pose_nonce') ) return;
			endif;

			if ( (key_exists('post_type', $_POST)) && ('dt_yoga_poses' == $_POST['post_type']) ) :
		...
		function dt_template_include($template) {
			...
			if (is_singular( 'dt_yoga_poses' )) {
				if (! file_exists ( get_stylesheet_directory () . '/single-dt_yoga_poses.php' )) {
					$template = plugin_dir_path ( __FILE__ ) . 'templates/single-dt_yoga_poses.php';
				}
			}
